        ...
        return of(null).pipe(mergeMap(() => {
            ...
            if (request.url.endsWith('/users/authenticate') && request.method === 'POST') {
                ...
                if (filteredUsers.length) {
                    ...
                    if (user.lockflag === true) {
                      return throwError({ error: { message: 'User has been locked, please contact support team.' } });
                    }
                    const body = {
                        id: user.id,
            ...
            if (request.url.match(/\/users\/\d+$/) && request.method === 'GET') {
                if (request.headers.get('Authorization') === 'Bearer fake-jwt-token') {
                    ...
                    const matchedUsers = users.filter( item => { return item.id === id; });
            ...
            if (request.url.match(/\/users\/\d+$/) && request.method === 'PUT') {
              if (request.headers.get('Authorization') === 'Bearer fake-jwt-token') {
                ...
                users.forEach( user => {
                  if (user.id === id) {
                    user.lockflag = request.body.lockflag;
                    localStorage.setItem('users', JSON.stringify(users));
                    return;
                  }
                });
